...
void detectAndDisplay()
{
   ...
   if(mode_wire) {
    ...
    for( size_t i = 0; i < faces.size(); i++ ){
        ...
        //cout<<"headDis " <<headDis<<endl;
        
        // 2D image points
        imageCorners_temp.clear();
        imageCorners_temp.push_back(Point2f(faces[0].x,faces[0].y)); // left bottom
        imageCorners_temp.push_back(Point2f(faces[0].x,faces[0].y+faces[0].height)); // left top
    imageCorners_temp.push_back(Point2f(faces[0].x+faces[0].width,faces[0].y+faces[0].height)); // right top
        
        imageCorners_temp.push_back(Point2f(faces[0].x+faces[0].width,faces[0].y)); // right bottom

    }
       //cout<<"imageCorners_temp "<<imageCorners_temp<<endl;
   }
}
...
void idle(){
    ...
    if(frameCount==0){
        ...
        //cout<<"objCorners_temp "<<objCorners_temp<<endl;
    }
    
    solvePnP(objCorners_temp, imageCorners_temp, cameraMatrix, distortionCoefficients, rvec_temp, tvec_temp);
    
    //cout<<"rvec_temp "<<rvec_temp<<endl;
    //cout<<"tvec_temp "<<tvec_temp<<endl;
...
//        
    ...
    glPushMatrix();
    glTranslated(headWidth/2, headWidth/2, 0);

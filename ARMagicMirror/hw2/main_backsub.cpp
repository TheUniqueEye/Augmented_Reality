...
void detectAndDisplay()
{
   ...
   if(mode_wire) {
    ...
    // Computing the distance from head to camera? cm? pixel? /// > . < /// !
    
    /*
    float headHeight = 22;
    float Z = fy * headHeight / (720 - faces[0].y - cy);
    
    cout<<"face width " <<faces[0].width<<endl;
    cout<<"face height " <<faces[0].height<<endl;
    cout<<"face x " <<faces[0].x<<endl;
    cout<<"face y " <<faces[0].y<<endl;
    cout<<"Z" <<Z<<endl;
    cout<<"cy" <<cy<<endl;
    cout<<"fy" <<fy<<endl;
    */
...
void idle(){
    ...
    //cout<< "touched"<< buttonTouched(50,150,550,650) <<endl;
    if(touchCount>12){
...
void display(){
    ...
    glPushMatrix();
    glTranslated(0, 0, -10);
    glTranslatef(faces[0].x * resize_ratio * 0.03-2 , faces[0].y * resize_ratio *0.03-1,  -faces[0].width * resize_ratio * 0.05);
